...
function adRewardErrorResponse(error: unknown, fallback: string) {
  ...
  const message =
    code === "ad_provider_disabled"
      ? "광고를 불러올 수 없습니다."
      : code === "ad_daily_limit"
        ? "오늘 광고 보상을 모두 사용했습니다."
        : code === "ad_cooldown"
          ? "아직 광고 보상 쿨다운 중입니다. 잠시 후 다시 시도해주세요."
          : code === "ad_related_reward_exists"
            ? "이미 처리 중이거나 완료된 광고 보상입니다."
            : code === "no_forge_reward"
              ? "아직 수령할 제련의 불씨가 없습니다."
              : code === "weapon_not_found"
                ? "무기를 찾을 수 없습니다. 서버 데이터를 다시 불러와주세요."
                : code === "locked_weapon"
                  ? "잠긴 무기는 판매할 수 없습니다."
                  : code === "destroyed_weapon"
                    ? "이미 파괴된 무기입니다."
                    : code === "reward_type_not_ready"
